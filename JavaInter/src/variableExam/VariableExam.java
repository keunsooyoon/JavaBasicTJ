...
/*
변수 variable 란!! "단 하나의 값을 저장할수 있는 메모리 공간"

변수를 사용하기 위해서는 먼저 초기화 해야 한다. 

       변수이름 : 메모리 공간에 이름을 붙인다. 같은 이름이 여러개 존재할수 없다. 
int age;    // age라는 이름의 변수를 선언 
변수타입 : 변수에 저장될 값의 종류를 지정하는 것. 정수int 실수double (문자 String).....
...
변수 타입
 ...
 값(data)의 종류(type)에 따라 저장될 공간의 크기와 형식을 정하는것이
 자료형 
...
변수와 마찬가지로 값을 저장할수 있는 공간 변수와는 달리 한번 값을 저장하면
변경하지 않고 사용한다. 선언하는 방법도 변수와 동일하지만 final을 붙여서 상수를 나타낸다. 
관습적으로 상수는 대문자로 상수명을 표기한다. 

final int MAX_SPEED = 100; // 상수를 final로 선언하면 변경불가능

상수 선언은 반드시 선언과 동시에 초기화 해야만 한다. 
final int MIN_SPEED; (x)
MIN_SPEED = 30; (x) 상수의 값은 변경할 수 없다. 

상수의 이름 모두 대문자로 하는것이 암묵적인 관례이다. 

int i = 'A'; //'문자'A' 코드 값이 저장된다. 
long l = 123; //int 의 크기 보다 long 크기가 더 크다. 
double d = 3.14f; //double 의 크기가 float보다 더 크다. 

float f = 3.14; // 에러...double 을 float에 넣을수 없다.
                // 크기와는 상관없이 무조건 double은 float에 넣을수 없다. 
int i  = 43627146321644321431432139832L; //에러...long이 int보다 크다.
...
다른 형태로는 출력하기가 힘들다. 예를 들면 소수 2자리까지만 출력...
prinf는 println과 달리 줄바꿈을 하지 않는다. 
줄바꿈을 할때는 %n 을 넣는다. 
